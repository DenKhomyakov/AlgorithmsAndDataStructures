...
public:
	...
	virtual int GetMinKey(Node* node) const;									// Получение минимального ключа дерева
	int GetMinKey() const;

	virtual int GetMaxKey(Node* node) const;									// Получение максимального ключа дерева
	int GetMaxKey() const;

	virtual Node* AddNode(Node* node, const int key);							// Добавление узла в дерево
	void AddNode(const int key);

	virtual bool RemoveNode(Node*& node, const int key);						// Удаление узла из дерева по ключу
	...
	int GetSize() const;														// Получение размера дерева (количество узлов)
