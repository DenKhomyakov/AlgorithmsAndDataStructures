...
int BinaryTree::GetMinSearchKey(Node* node) const {
	if (node == nullptr) {
		return INT_MAX;
	}

	int leftMin = GetMinSearchKey(node->GetLeft());
	int rightMin = GetMinSearchKey(node->GetRight());

	return std::min(node->GetKey(), std::min(leftMin, rightMin));
}

int BinaryTree::GetMinSearchKey() const {
	return GetMinSearchKey(m_root);
}

/* Получение максимального ключа дерева */
int BinaryTree::GetMaxSearchKey(Node* node) const {
	if (node == nullptr) {
		return INT_MIN;
	}

	int maxKey = node->GetKey();
	int leftMaxKey = GetMaxSearchKey(node->GetLeft());
	int rightMaxKey = GetMaxSearchKey(node->GetRight());
...
int BinaryTree::GetMaxSearchKey() const {
	return GetMaxSearchKey(m_root);
}

/* Добавление узла в дерево */
void BinaryTree::AddSearchNode(const int key) {
	m_root = AddSearchNode(m_root, key);
}

BinaryTree::Node* BinaryTree::AddSearchNode(Node* node, const int key) {
	if (!node) {
		node = new Node(key);
	}
	else if (rand() % 2) {
		node->SetLeft(AddSearchNode(node->GetLeft(), key));
	}
	else {
		node->SetRight(AddSearchNode(node->GetRight(), key));
	}
